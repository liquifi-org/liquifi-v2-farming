...
contract StakingFactoryV2 is IStakingFactory, OwnableUpgradeable {
    ...
    function upgrade(address _proxy) external onlyOwner {
        InitializableAdminUpgradeabilityProxy(payable(_proxy)).upgradeTo(getOrCreateImplementation());
    }
    ...
    function _createStaking(address _token, address _minter) internal returns (address) {
        bytes memory data = abi.encodeWithSelector(StakingV22.initialize.selector, _token, _minter, owner());
    ...
    function getOrCreateImplementation() internal returns (address implementation) {
        ...
        bytes memory bytecode = type(StakingV22).creationCode;
