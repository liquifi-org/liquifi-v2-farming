...
export const advanceBlockTo = async (provider: ethers.providers.JsonRpcProvider, number: number) => {
  for (let i = await getCurrentBlock(provider); i < number; i++) await provider.send("evm_mine", []) // mine the next block
}

export const getCurrentBlock = async (provider: ethers.providers.JsonRpcProvider) => provider.getBlockNumber()
